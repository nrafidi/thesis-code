...
if __name__ == '__main__':
    ...
    if args.experiment == 'krns2':
        if args.word == 'senlen':
            raise ValueError('Not a valid word for this experiment')
    if sen_type != 'pooled':
        if word in ['senlen', 'noun1', 'voice', 'propid']:
            raise ValueError('Not a valid word for this sen_type')
    ...
    num_subs = len(run_coef_TGM_EOS_multisub.VALID_SUBS[args.experiment])
    ...
    num_plots = int(ceil(float(num_subs)/float(n_rows)))
    ...
    for i_sub in range(num_subs):
        ...
        ax.set_title('%s' % (run_coef_TGM_EOS_multisub.VALID_SUBS[args.experiment][i_sub]), fontsize=axistitlesize)

        ax.text(-0.13, 1.075, string.ascii_uppercase[i_sub], transform=ax.transAxes,
                size=axislettersize, weight='bold')

    cbar = combo_grid.cbar_axes[0].colorbar(im)

    combo_fig.suptitle('Subject Importance Maps for %s at %.2f s Post-Onset' % (word, args.time_to_plot),
        fontsize=suptitlesize)
    combo_fig.savefig(
            '/home/nrafidi/thesis_figs/{exp}_eos_helmet{id}_{tplot}_sub_{sen_type}_{word}_{alg}_win{win_len}_ov{overlap}_ni{num_instances}_avgTime{avgTime}_avgTest{avgTest}.pdf'.format(
    ...
    combo_fig.savefig(
        '/home/nrafidi/thesis_figs/{exp}_eos_helmet{id}_{tplot}_sub_{sen_type}_{word}_{alg}_win{win_len}_ov{overlap}_ni{num_instances}_avgTime{avgTime}_avgTest{avgTest}.png'.format(
