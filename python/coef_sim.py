...
if __name__ == '__main__':
    ...
    for i_win, res in enumerate(sub_results):
        ...
        for i_coef in range(1, len(res)):
            ...
            prev_mat = np.sum(np.reshape(np.sum(prev_res, axis=0), (win_size, 306)), axis=0)
            prev_mat = np.array(prev_mat != 0).astype('float')
            curr_mat = np.sum(np.reshape(np.sum(curr_res, axis=0), (win_size, 306)), axis=0)         
            curr_mat = np.array(curr_mat != 0).astype('float')
  ...
  #              plt.show()
    ...
    meow = np.mean(sim_mat, axis=1)
    print(meow)
    woof = matlib.repmat(meow[:, None], 1, sim_mat.shape[1])
    print(woof.shape)
    sim_mat -= woof
    f, ax = plt.subplots()
