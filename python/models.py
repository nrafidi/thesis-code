...
def _json_output_from_preds(output_path, preds,labels, labels_to_index, cv_scheme, win_len, window_starts):
  cv_scheme = np.array(cv_scheme)
  labels = np.array(labels)
  n_w = preds[0].shape[0]
  tgm = np.zeros((n_w,n_w,2)) # rows define train time
  ...
  for i in xrange(n_w):
    for cvi in xrange(n_cv):
      ...
      p = np.argmax(preds[cvi][i],axis=1)
      n_pred = p.shape[0]
      tgm[i,:,0] = tgm[i,:,0] + p == l_indicies
      tgm[i,:,1] = tgm[i,:,1] + n_pred

  tgm = tgm[:,:,0]/tgm[:,:,1]
  c_dist =  list([list(c) for c in Counter(labels).iteritems()])
  max_acc = np.max(tgm)


  json_obj = { 'max_accuracy' : max_acc,
    'tgm' : tgm.tolist(), 
    'window_starts' : window_starts,
    'window_length' : win_len,
    'class_distribution' : c_dist,
    'preds' : [[x.tolist() for x in p] for p in preds]
  }
...
def _nb_tgm(data, labels, cv_scheme, win_starts, win_len, usis, feature_select=None, feature_select_params=None, dozscore=False,ddof=1,doAvg=False):
 # if dozscore==True:
      ...
      if feature_select is not None:
        if feature_select == 'distance_of_means':
          mu_diff = reduce(lambda accum, lis :  accum + np.abs(mu_full[lis[0]] - mu_full[lis[1]]), 
            ((li1, li2) for li1 in xrange(n_l) for li2 in xrange(li1+1, n_l)), np.zeros(mu_full[0].shape))
...
def _lr_tgm(data, labels, cv_scheme, win_starts, win_len, dozscore=False,ddof=1,doAvg=False):
...
'''
  unls : list of (identifier, list of UNLs)
  P_c =
  labels : list of string, length of unls
  cv_scheme : membership of CV scheme, -1 if not in any test
'''
def load_data(ids_uels, tmin, tmax, preproc_slug, baseline=None):
  identifiers, uels  = zip(*ids_uels)

  evokeds = [hippo.io.load_mne_epochs(us, preprocessing=preproc_slug, baseline=baseline,
    tmin=tmin, tmax=tmax).average() for us in uels]

  return evokeds

def load_data_noAvg(ids_uels, tmin, tmax, preproc_slug, baseline=None):
  identifiers, uels  = zip(*ids_uels)
...
class NaiveBayesModel(object):
  ...
  def __init__(s, id_uels, labels, cv_scheme, tmin, tmax, windows, preproc_slug, baseline=None):
    s.id_uels = id_uels
    s.labels = labels
    s.cv_scheme = cv_scheme
    s.tmin = tmin
    s.tmax = tmax
    s.window = windows
    s.preproc_slug = preproc_slug
    s.baseline = baseline
...
# Old code for zscore
 #     mu_full = np.array([np.mean(
 #           np.divide(
 #             data[np.logical_and(in_train, in_l[li]),:,:]-mu_full_all[None,:,:],
 #             std_full[None,:,:]
 #           ),
 #           0) for li in xrange(n_l)])
 #     double_var_full = 2*np.var(
 #           np.divide(
 #             data[in_train,:,:]-mu_full_all[None,:,:],
 #             std_full[None,:,:]
#            ),0)
 #     if dozscore:
 #       P_cGx = np.array([
 #           np.sum(
 #             np.multiply(np.divide(data[in_test,:,:][:,:,t_w] - mu_full_all[None,:,train_time], std_full[:,train_time])[:,None,:,:],A[None,:,:,:]),
 #             axis=(2,3)
 #           ) - B
 #           for t_w in test_windows])
#      else:
