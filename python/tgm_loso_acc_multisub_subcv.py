...
if __name__ == '__main__':
    ...
    for i_sen, sen_type in enumerate(['active', 'passive']):
        for i_word, word in enumerate(word_list):
           ...
           # acc_all = result['tgm_acc']
            ...
            mean_acc = np.mean(acc_all, axis=1)
            ...
            print(i_combo)
            im = ax.imshow(frac_above, interpolation='nearest', aspect='auto', vmin=0.5, vmax=1.00)
    ...
    combo_fig.savefig('/home/nrafidi/thesis_figs/{exp}_avg-tgm-frac{tsss}_multisub_{sen_type}_{word}_{alg}_win{win_len}_ov{overlap}_ni{num_instances}_avgTime{avgTime}_avgTest{avgTest}{rep_str}.pdf'.format(
    ...
    combo_fig.savefig(
        '/home/nrafidi/thesis_figs/{exp}_avg-tgm-frac{tsss}_multisub_{sen_type}_{word}_{alg}_win{win_len}_ov{overlap}_ni{num_instances}_avgTime{avgTime}_avgTest{avgTest}{rep_str}.png'.format(
            exp=args.experiment, sen_type='both', word='all', alg=args.alg, avgTime=args.avgTime, avgTest=args.avgTest,
            win_len=args.win_len,
            overlap=args.overlap,
            tsss=tsss_str,
