...
def run_tgm_exp(experiment,
    ...
    for i_sub, subject in enumerate(VALID_SUBS[experiment]):
        ...
        labels_list_sub = [n1_labels, n2_labels]
        win_starts_list_sub = [n1_win_starts, n2_win_starts]
        sen_ints_list_sub = [n1_sen_ints, n2_sen_ints]
        if i_sub == 0:
            labels_list=labels_list_sub
            win_starts_list=win_starts_list_sub
            sen_ints_list=sen_ints_list_sub
        else:
            assert np.all(np.array(labels_list) == np.array(labels_list_sub))
    ...
    np.savez_compressed(fname,
                        ...
                        tgm_acc=tgm_acc,
                        tgm_pred=tgm_pred,
                        n1_win_starts=win_starts_list[0],
                        n2_win_starts=win_starts_list[1],
                        n1_time=n1_time,
                        n2_time=n2_time,
                        proc=proc)
