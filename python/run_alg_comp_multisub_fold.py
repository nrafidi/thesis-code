...
def tgm_from_preds_GNB(preds, l_ints, cv_membership):
    ...
    tgm_corr = np.zeros((1, num_win, num_win))
    tgm_total = np.zeros((1, num_win, num_win))
    for fold in range(num_folds):
        ...
        for i_win in range(num_win):
            for j_win in range(num_win):
                ...
                # print(yhat.shape)
                tgm_corr[0, i_win, j_win] += np.sum(yhat == labels)
                tgm_total[0, i_win, j_win] += preds[fold, i_win, j_win].shape[0]
...
def run_tgm_exp(experiment,
    ...
    if 'lr' in alg:
        ...
        l_ints, cv_membership, tgm_acc, tgm_pred = models.svm_tgm_loso_multisub_fold(data_list,
                                                                                    labels,
                                                                                    win_starts,
                                                                                    win_len,
                                                                                    sen_ints,
        ...
        if 'None' in alg:
            ...
            doFeatSelect=True
        tgm_pred, l_ints, cv_membership, feature_masks, num_feat_selected = models.nb_tgm_loso_multisub_fold(data_list,
                                                                                                            labels,
                                                                                                            sen_ints,
                                                                                                            1,
                                                                                                            win_starts,
