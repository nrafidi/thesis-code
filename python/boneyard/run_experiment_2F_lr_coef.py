...
def pooled_krns2_lr_coef(experiment,subject, proc_slug,word,win_len,actORpass='pooled',doZscore=False,ddof=1,doAvg=False,do2Samp=False):
# Gets the stimulus IDs
  ...
  #Run TGM
  masks = hippo.models._lr_tgm_coef(data, labels, win_starts, win_len,doZscore,ddof,doAvg)
  ...
  if not os.path.exists(subDir):
    os.mkdir(subDir)
  fnameStub = subDir + '/TGM_coef_{sub}_{word}_pooled_win{win_len}_zscore{doZ}_avg{doAvg}_2Samp{do2Samp}_2F_LR.mat'
  scipy.io.savemat(fnameStub.format(sub=subject,word=word,win_len=win_len,doZ=doZscore,doAvg=doAvg,do2Samp=do2Samp), mdict={'feature_masks':masks, 'labels':labels})

# Pooled
###################################################################################################################################

# Runs the TGM experiment, 2F CV, separately for active and passive sentences
def actORpass_krns2_lr_coef(experiment,subject, proc_slug,word,win_len,actORpass,doZscore=True,ddof=1,doAvg=False,do2Samp=True):
...
#  scipy.io.savemat('/home/nrafidi/evokedDataTest.mat',mdict={'data':data,'cv':cv,'labels':labels})
  ...
  #Run TGM
  masks = hippo.models._lr_tgm_coef(data, labels, win_starts, win_len,doZscore,ddof,doAvg=doAvg)
  ...
  fnameStub = subDir + '/TGM_coef_{sub}_{word}_{aorp}_win{win_len}_zscore{zscore}_avg{doAvg}_2F_LR.mat'
  scipy.io.savemat(fnameStub.format(sub=subject,word=word,aorp=actORpass,win_len=win_len,zscore=doZscore,doAvg=doAvg), mdict={'feature_masks':masks,'labels':labels})
...
#  parser.add_argument('--doPDTW',required=False,type=int)
  ...
  {
    'actORpass_krns2_lr_coef' : actORpass_krns2_lr_coef,
    'pooled_krns2_lr_coef' : pooled_krns2_lr_coef,
