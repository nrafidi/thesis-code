...
import os.path
import numpy as np
import batch_experiments_multisub_subcv_fold as batch_exp

TOP_DIR = '/share/volume0/nrafidi/{exp}_TGM_LOSO/'
SAVE_FILE = '{dir}TGM-LOSO_multisub-less{sub}_{sen_type}_{word}_win{win_len}_ov{ov}_pr{perm}_' \
            'alg{alg}_adj-{adj}_avgTime{avgTm}_avgTest{avgTst}_ni{inst}_' \
            'rsPerm{rsP}_{mode}'

TOTAL_SAVE_FILE = '{dir}TGM-LOSO_multisub-subcv_{sen_type}_{word}_win{win_len}_ov{ov}_pr{perm}_' \
            'alg{alg}_adj-{adj}_avgTime{avgTm}_avgTest{avgTst}_ni{inst}_' \
            'rsPerm{rsP}_{mode}'
NEW_SAVE_FILE = '{dir}TGM-LOSO_multisub-less{sub}_{sen_type}_{word}_win{win_len}_ov{ov}_pr{perm}_' \
            'alg{alg}_adj-{adj}_avgTime{avgTm}_avgTest{avgTst}_ni{inst}_' \
            'rsPerm{rsP}_{fold}'
    ...
    for grid in param_grid:
        ...
        for i_sub, sub in enumerate(batch_exp.SUBJECTS):
            ...
            if os.path.isfile(complete_job + '.npz'):
                ...
                cv_membership.append(total_result['cv_membership'])
