...
echo $force
python run_TGM_KF_EOS_multisub_fold.py --experiment $experiment --sen_type $sen_type --word $word --win_len $win_len --overlap $overlap \
--isPerm $isPerm --adj $adj --alg $alg --doTimeAvg $doTimeAvg --doTestAvg $doTestAvg --fold $fold \
--num_folds $num_folds --num_instances $num_instances --cv_random_state $cv_random_state --perm_random_state $perm_random_state --force $force \
